...
def get_datasets(repo_id: str, revision: str | None = None) -> set[str]:
...
def get_unannotated_datasets(repo_id: str) -> list[str]:
    ...
    dataset_folders = get_datasets(repo_id)

    # fetch open PRs to avoid duplicate annotations
    open_prs = api.get_repo_discussions(repo_id, repo_type="dataset")
...
def create_pr(repo_id: str, local_path: str, remote_path: str, commit_message: str, description: str = ""):
    api.upload_file(
